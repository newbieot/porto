(() => {
    ...
    // Triliun rupiah. Juni 2026 tersedia untuk kelima bank.
    // Juli 2026 tersedia untuk BBCA dan BMRI dengan pembanding Juli 2025.
    // BNGA Juni memakai basis bank individual: kredit konvensional 2026 dihitung
    // dari total kredit Rp235,927720 T dikurangi pembiayaan syariah bruto Rp54,042137 T.
    const data2025 = {
        credit: {
    ...
    const bbcaJuly = {
        profit: 35.252398,
        profitPrior: 34.707591,
        credit: 1000.882825,
        creditPrior: 923.512447,
        assets: 1575.898313,
        assetsPrior: 1466.750654,
        equity: 266.555525,
        equityPrior: 257.754691,
        netInterestIncome: 46.705995,
        netInterestIncomePrior: 46.551608,
        deposits: 1257.725601,
        depositsPrior: 1162.055997,
        casa: 85.43,
        casaPrior: 83.40,
        derivedLdr: 79.58,
        derivedLdrPrior: 79.47,
        comprehensiveIncome: 33.191833,
        comprehensiveIncomePrior: 35.750545
    };
            ...
            note.textContent = state.selectedBank === 'BBNI'
                ...
                : state.selectedBank === 'NISP'
                    ...
                    : state.selectedBank === 'BBCA'
                        ? 'BBCA telah diperbarui sampai Juli 2026 dengan laporan Juli 2025 sebagai pembanding YoY. Angka bulanan berasal dari laporan bank individual; DPK, CASA, dan LDR merupakan hasil hitung.'
                        : state.selectedBank === 'BMRI'
                            ...
                            : state.selectedBank === 'BNGA'
                                ? 'BNGA telah diperbarui sampai Juni 2026. Laba memakai bank individual; kredit konvensional Juni 2026 merupakan hasil hitung dari total kredit bank individual dikurangi pembiayaan syariah bruto agar konsisten dengan seri Januari-Mei.'
                            : state.selectedBank === 'all'
                                ? 'Agregat lima bank kini lengkap dan dapat dibandingkan sampai Juni 2026; Juli tersedia untuk BBCA dan BMRI.'
                                : `${state.selectedBank} telah diperbarui sampai Juni 2026; laporan Juli belum ditambahkan.`;
        }
    ...
    function renderSummary() {
        const profitGrowth = growth(bbcaJuly.profit, bbcaJuly.profitPrior);
        const creditGrowth = growth(bbcaJuly.credit, bbcaJuly.creditPrior);
        document.getElementById('latest-profit').textContent = formatGrowth(profitGrowth, 'YoY').replace(' YoY', '');
        document.getElementById('latest-profit-detail').textContent = `BBCA Jan-Jul net profit · ${formatTrillion(bbcaJuly.profit)}`;
        document.getElementById('latest-loan').textContent = formatTrillion(bbcaJuly.credit);
        document.getElementById('latest-loan-detail').textContent = `BBCA July loans · ${formatGrowth(creditGrowth)}`;
    }
