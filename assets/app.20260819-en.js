(() => {
    ...
    function setLoading(isLoading) {
        ...
        if (isLoading) {
            setStatus('is-loading', 'Connecting');
    ...
    function recordsFromCsv(text) {
        ...
        if (rows.length < 2) throw new Error('The Google Sheet did not return usable rows.');
        ...
        if (!normalized.length) throw new Error('No valid portfolio records were found.');
        return normalized;
    }
    ...
    async function loadData({ silent = false } = {}) {
        ...
        setLoading(true);
        if (!silent) els.latestDetail.textContent = 'Reading the published portfolio dataset…';
        ...
        try {
            ...
            if (!text) throw finalError || new Error('The dataset could not be retrieved.');

            state.records = recordsFromCsv(text);
            renderDashboard();
            setStatus('is-online', 'Live CSV');
            els.performanceLoading.hidden = true;
            if (!silent) showToast('Portfolio data has been refreshed.');
        } catch (error) {
            console.error(error);
            setStatus('is-error', 'Data unavailable');
            els.latestDate.textContent = 'Unable to connect';
            els.latestDetail.textContent = 'Check the published Google Sheet or try again shortly.';
            els.performanceLoading.innerHTML = '<span></span>Data unavailable — use Refresh data to try again.';
            els.performanceLoading.hidden = false;
            if (!silent) showToast('The live dataset could not be loaded.');
        } finally {
            state.loading = false;
    ...
    function renderDashboard() {
        ...
        setMetric(els.records, new Intl.NumberFormat('en-GB').format(state.records.length));

        els.latestDate.textContent = formatLongDate(latest.date);
        els.latestDetail.textContent = `Latest values: net ${formatPercent(latest.net)} and unrealized ${formatPercent(latest.floating)}.`;
    ...
    function renderPerformanceChart() {
        ...
        const datasets = [
            chartDataset('Net return', rebase(records.map(record => record.net)), '#20c98a', { fill: true, width: 2.6 }),
            chartDataset('Unrealized P/L', rebase(records.map(record => record.floating)), '#f06472', { dash: [6, 5], width: 2.2 }),
        ...
        state.performanceChart = new Chart(document.getElementById('performance-chart'), {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { grid: { color: colors.grid }, ticks: { color: colors.soft, callback: value => `${Number(value).toLocaleString('en-GB', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%`, font: { size: 10 } }, border: { display: false } }
                }
            }
        });
    }
    ...
    function renderAllocation() {
        ...
        list.replaceChildren(...allocation.map(item => {
            ...
            value.textContent = `${item.value.toLocaleString('en-GB', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%`;
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        borderColor: colors.border,
                        borderWidth: 1,
                        callbacks: { label: context => `${context.label}: ${context.raw.toLocaleString('en-GB', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%` }
                    }
                }
            }
        });
    ...
    function renderKeyStats() {
        const series = [
            { name: 'Net return', key: 'net', color: '#20c98a' },
            { name: 'Unrealized P/L', key: 'floating', color: '#f06472' },
    ...
    function toggleHistory() {
        ...
        els.historyToggle.textContent = expanded ? 'Show all events' : 'Show fewer events';
    ...
    function init() {
        ...
        if (!window.Chart) {
            setStatus('is-error', 'Chart library unavailable');
            els.latestDate.textContent = 'Charts could not start';
            els.latestDetail.textContent = 'Refresh the page or check the external Chart.js source.';
            return;
        }
