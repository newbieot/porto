(() => {
    ...
    // Trillion IDR. June 2026 is currently available for BBCA and BMRI.
    const data2025 = {
        credit: {
    ...
    const updatedJuneBanks = new Set(['BBCA', 'BMRI']);
    ...
    function renderCharts() {
        ...
        if (note) {
            note.textContent = state.selectedBank === 'BBCA'
                ...
                : state.selectedBank === 'BMRI'
                    ...
                    : state.selectedBank === 'all'
                        ? 'June is intentionally blank in the aggregate because BBCA and BMRI are updated while three peer reports remain pending. The five-bank aggregate remains comparable through May.'
    ...
    function renderSummary() {
        const profitGrowth = growth(bbcaQ2.profit, bbcaQ2.profitPrior);
        const creditGrowth = growth(bbcaQ2.credit, bbcaQ2.creditPrior);
        document.getElementById('latest-profit').textContent = `+${profitGrowth.toFixed(1)}%`;
        document.getElementById('latest-profit-detail').textContent = `BBCA H1 net profit · ${bbcaQ2.profit.toFixed(2)}T`;
        document.getElementById('latest-loan').textContent = `${bbcaQ2.credit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}T`;
        document.getElementById('latest-loan-detail').textContent = `BBCA June credit · +${creditGrowth.toFixed(1)}% YoY`;
    }
