(() => {
    ...
    function recordsFromCsv(text) {
        ...
        if (rows.length < 2) throw new Error('The Google Sheet returned no usable rows.');
    ...
    async function loadData({ silent = false } = {}) {
        ...
        try {
            ...
            if (!text) throw finalError || new Error('Unable to retrieve the dataset.');

            state.records = recordsFromCsv(text);
            renderDashboard();
            setStatus('is-online', 'Live CSV');
            els.performanceLoading.hidden = true;
            if (!silent) showToast('Portfolio data updated.');
        } catch (error) {
            console.error(error);
            setStatus('is-error', 'Data unavailable');
            els.latestDate.textContent = 'Unable to connect';
            els.latestDetail.textContent = 'Check the published Google Sheet or try again in a moment.';
            els.performanceLoading.innerHTML = '<span></span>Data unavailable — use Refresh data to retry.';
            els.performanceLoading.hidden = false;
    ...
    function renderDashboard() {
        ...
        setMetric(els.records, new Intl.NumberFormat('en-US').format(state.records.length));

        els.latestDate.textContent = formatLongDate(latest.date);
        els.latestDetail.textContent = `Latest recorded values: net ${formatPercent(latest.net)} and floating ${formatPercent(latest.floating)}.`;
    ...
    function renderPerformanceChart() {
        ...
        const datasets = [
            ...
            chartDataset('Floating P&L', rebase(records.map(record => record.floating)), '#f06472', { dash: [6, 5], width: 2.2 }),
        ...
        state.performanceChart = new Chart(document.getElementById('performance-chart'), {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { grid: { color: colors.grid }, ticks: { color: colors.soft, callback: value => `${Number(value).toFixed(1)}%`, font: { size: 10 } }, border: { display: false } }
                }
            }
        });
    }
    ...
    function renderKeyStats() {
        const series = [
            ...
            { name: 'Floating P&L', key: 'floating', color: '#f06472' },
    ...
    function init() {
        ...
        if (!window.Chart) {
            ...
            els.latestDetail.textContent = 'Refresh the page or check the external Chart.js resource.';
            return;
        }
